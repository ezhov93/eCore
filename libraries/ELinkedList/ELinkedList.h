/*
	ELinkedList.h - V1.1 - Generic ELinkedList implementation
	Works better with FIFO, because LIFO will need to
	search the entire List to find the _last one;

	For instructions, go to https://github.com/ivanseidel/ELinkedList
...
private:
  struct ListNode {	T data; ListNode *next; };
	int _size;
	ListNode *_root;
	ListNode	*_last;
...
template<typename T>
void ELinkedList<T>::fill(T value){
	for (int i = 0; i < size(); i++){
		setValue(i, value);
	}
}
template<typename T>
int ELinkedList<T>::indexOf(T value) {
   for (int i=0; i<size(); ++i)
      if(at(i) == value)
        return i;
   return -1;
...
template<typename T>
void ELinkedList<T>::append(int index, T value){
	if(index >= _size)
		return append(value);
	if(index == 0)
...
template<typename T>
void ELinkedList<T>::append(T value){
	ListNode *tmp = new ListNode();
	tmp->data = value;
	tmp->next = NULL;
...
template<typename T>
void ELinkedList<T>::setValue(int index, T value){
...
template<typename T>
void ELinkedList<T>::remove(int index){
	if (index < 0 || index >= _size)
		return T();
...
template<typename T>
void ELinkedList<T>::sort(int (*cmp)(T &, T &)){
	if(_size < 2) return;
...
template<typename T>
typename ELinkedList<T>::ListNode* ELinkedList<T>::findEndOfSortedString(
  ListNode *p, int (*cmp)(T &, T &)) {
